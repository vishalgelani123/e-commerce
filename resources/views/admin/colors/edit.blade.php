...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" id="colors-form" action="{{ route('admin.colors.update', [$color->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.color.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $color->name) }}" pattern="^#+([a-fA-F0-9]{6}|[a-fA-F0-9]{3})$" required>
                @if($errors->has('name'))
