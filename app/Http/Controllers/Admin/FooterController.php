...
class FooterController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $query = Footer::query()->select(sprintf('%s.*', (new Footer())->table));
            ...
            $table->addColumn('actions',function($row){
                ...
                $btn .= ' <a class="btn btn-sm btn-warning my-1" href="' . route('footer.edit', [$row->id]) . '"> <i class="fas fa-edit"></i></a>';
                return $btn;
            });
            ...
            $table->rawColumns(['actions', 'placeholder','id','name','status']);
    ...
    public function edit($id)
    {
        ...
        return view('admin.footer.edit', compact('slider'));
    }
    ...
    public function updateData(Request $request,$id)
    {
        ...
        $result = DB::table('footers')->where('id',$id)->update(array('title'=>$request->title));
        // $post = Footer::find($id);
    
        // $post->update($request->all());
    
        return redirect()->route('add-footer')
