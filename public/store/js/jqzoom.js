	...
	$.fn.zoom = function(options){
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".bzoom_big_image").length){
			...
			if(!$(".bzoom_magnifier").length){
				$bzoom_magnifier = $('<li class="bzoom_magnifier"><div class=""><img onerror="handleError(this);"src="" /></div></li>');
			...
			if(!$('.bzoom_zoom_area').length){
                $bzoom_zoom_area = $('<li class="bzoom_zoom_area"><div><img onerror="handleError(this);"class="bzoom_zoom_img" /></div></li>');
			...
			if(!$(".bzoom_small_thumbs").length){
				...
				for(var i=0; i<_option.small_thumbs; i++){
					...
					if(i==0){
						html += '<li class="bzoom_smallthumb_active"><img onerror="handleError(this);"src="'+smurl+'" alt="small" style="width:'+smwidth+'px; height:'+smheight+'px;" /></li>';
					}else{
						html += '<li style="opacity:0.4;"><img onerror="handleError(this);"src="'+smurl+'" alt="small" style="width:'+smwidth+'px; height:'+smheight+'px;" /></li>';
					}
				}
