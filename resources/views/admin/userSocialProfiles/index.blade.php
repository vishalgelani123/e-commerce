...
@endpush
@section('content')
{{-- @can('user_social_profile_create') --}}
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-primary" href="{{ route('admin.user-social-profiles.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.userSocialProfile.title_singular') }}
            </a>
        </div>
    </div>
{{-- @endcan --}}
<div class="card">
    <div class="card-header">
        {{ trans('cruds.userSocialProfile.title_singular') }} {{ trans('global.list') }}
    </div>
...
@can('user_social_profile_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.user-social-profiles.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.user-social-profiles.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'user_name', name: 'user.name' },
{ data: 'social_profile_type_name', name: 'social_profile_type.name' },
      ...
      $($.fn.dataTable.tables(true)).DataTable()
          .columns.adjust();
            ...
            $(document).on('click','#is-attribute-chk', function(){
                ...
                $.ajax({
                    type:'POST',
                    url:"{{ route('admin.social-profiles.status.update') }}",
