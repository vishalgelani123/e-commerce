    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-variations.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('mrp_price') ? 'is-invalid' : '' }}" type="number" name="mrp_price" id="mrp_price" value="{{ old('mrp_price', '0.00') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('sales_price') ? 'is-invalid' : '' }}" type="number" name="sales_price" id="sales_price" value="{{ old('sales_price', '0.00') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('in_stock') ? 'is-invalid' : '' }}" name="in_stock" id="in_stock" required>
                    ...
                    @foreach(App\Models\ProductVariation::IN_STOCK_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('in_stock', '1') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
