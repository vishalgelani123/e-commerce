...
@section('content')
                                ...
                                @foreach($banners as $banner)
                                    @if($loop->iteration==1)
                                        <div class="item active">
                                           <img onerror="handleError(this);"style="max-width: 100%;" src="{{ $banner->photo->url}}" alt="Slider">
                                        </div>
                                    @else
                                        <div class="item">
                                           <img onerror="handleError(this);"style="max-width: 100%;" src="{{ $banner->photo->url}}" alt="Slider">
                        ...
                        <div class="carousel-inner" role="listbox">
                            @if(count($banners)>0)
                                @foreach($banners as $banner)
                                    @if($loop->iteration==1)
                                        <div class="item active">
                                           <img onerror="handleError(this);"style="max-width: 100%;" src="{{ asset('storage/banner/'.$banner->photo->thumb.'')}}" alt="Slider">
                                        </div>
                                    @else
                                        <div class="item">
                                           <img onerror="handleError(this);"style="max-width: 100%;" src="{{ asset('storage/banner/'.$banner->photo->thumb.'')}}" alt="Slider">
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="row">
                        @php
                        $newmodels=App\Models\HomeBestSeller::where('type',1)->skip(0)->take(2)->get();
                        $thirdmodel=App\Models\HomeBestSeller::where('type',1)->skip(2)->take(1)->first();
                        $fourmodel=App\Models\HomeBestSeller::where('type',2)->first();
                        @endphp
                      
                        @foreach ($newmodels as $mode)
                        <div class="side-img col-md-12">
                            <a href="{{ $mode->link ?? ''}}"><img onerror="handleError(this);"height="90%" class="img-fluid" src="{{ asset('storage/bannerimages').'/'.$mode->image }}" onerror="javascript:this.src='{{asset('front_assets/images/fmix.jpg')}}'"
                                alt="women" /></a>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="col-md-6">
                    <a href="{{ $thirdmodel->link ?? ''}}"><img onerror="handleError(this);"style="height: 504px;" class="img-fluid" src="{{ asset('storage/bannerimages').'/'.$thirdmodel->image }}"
                        alt="women" onerror="javascript:this.src='{{asset('front_assets/images/fsuit.jpg')}}'"/></a>
                </div>
            </div>
        </div>
    </section>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-xs-12">
                    <div class="row">
                        @php
                        $models=App\Models\HomeBestSeller::where('type',0)->get();
                    @endphp
                    @foreach ($models as $mode)
                        <div class="col-sm-6 col-xs-12">
                            <div class="catImg">
                             
                                <a href="{{ $mode->link ?? ''}}"><img onerror="handleError(this);"src="{{ asset('storage/bannerimages').'/'.$mode->image }}" onerror="javascript:this.src='{{asset('front_assets/images/bestsle0.jpg')}}'" class="img-responsive"></a>
                            </div>
                        </div>
                        @endforeach
                    </div>
    ...
    <section>
        ...
        <a href="{{ $mode->link ?? ''}}"><section class="vasvi-offer-banner"><img onerror="handleError(this);"src="{{ asset('storage/bannerimages').'/'.$fourmodel->image }}" alt="" /> </section></a>
