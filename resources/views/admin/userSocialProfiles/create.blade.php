...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" class="form-row" action="{{ route("admin.user-social-profiles.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group col-6">
                ...
                <select class="form-control select2 {{ $errors->has('social_profile_type') ? 'is-invalid' : '' }}" name="social_profile_type_id" id="social_profile_type_id" required>
                    @foreach($social_profile_types as $id => $entry)
                        <option value="{{ $id }}" {{ old('social_profile_type_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
